...
!function($) {
    ...
    $.fn.valid=function(options){
        ...
        $('input, textarea', this).each(function () {
            ...
            if (el.attr('disabled')!="disabled" && !controlGroup.hasClass('has-success') && valid != null && valid.length > 0) {
        ...
        if (globalOptions.reqmark==true){
            if(fform_style==0){
                $(obj).find(".form-group>label").each(function(){
                    ...
                    controlGroup.append('<span id="autoreqmark" style="color:#FF9966">*</span>')
                });
            }
                ...
                $(obj).find('input, textarea').each(function(){
                    ...
                    controlGroup.find("#valierr").remove();
                    valid = (el.attr('check-type')==undefined)?null:el.attr('check-type').split(' ');
                    if (el.attr('disabled')!="disabled" && valid){
                        if ($.inArray('required',valid)>=0){
                            controlGroup.append('<span class="help-block" id="valierr" style="color:#FF9966">*</span>');
                        }
                    };
                }); 
            };
